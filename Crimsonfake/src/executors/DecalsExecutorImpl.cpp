...
namespace Game::ExecutionBlock
{
    ...
    void Decals::initImpl() noexcept
    {
        ...
        for (int32_t tileId{}; tileId < needTextures; ++tileId)
        {
            ...
            Dod::DataUtils::populate(this->renderCmdsContext.createRenderTextureCmd, cmd, true);
        }
    ...
    void Decals::updateImpl([[maybe_unused]] float dt) noexcept
    {
        ...
        Dod::Algorithms::getSortedIndices(this->internalContext.sortedIndices, Dod::DataUtils::createImFromBuffer(textures));
        const auto sortedMaterials{ Dod::DataUtils::createSortedImBuffer(
            Dod::DataUtils::createImFromBuffer(textures),
            Dod::DataUtils::createImFromBuffer(this->internalContext.sortedIndices)
        ) };
        ...
        for (int32_t tileId{}; tileId < needTextures; ++tileId)
        {
            ...
            Dod::DataUtils::constructBack(this->renderCmdsContext.rtBatchTarget, rtkey, true);

            for (int32_t elId{}, batchId{}; elId < Dod::DataUtils::getNumFilledElements(this->internalContext.sortedIndices); ++batchId)
            {

                const auto material{ Dod::DataUtils::get(sortedMaterials, elId) };

                Dod::DataUtils::constructBack(this->renderCmdsContext.rtbatchDepth, 1, true);
                Dod::DataUtils::constructBack(this->renderCmdsContext.rtbatchMaterial, material, true);

                const auto numOfElementsPerBatch{ Dod::DataUtils::get(this->internalContext.elementsPerBatch, batchId) };
                Dod::DataUtils::constructBack(this->renderCmdsContext.rtbatches, Types::Render::BatchDesc(numOfElementsPerBatch), true);

                for (int32_t localElId{ elId }; localElId < numOfElementsPerBatch; ++localElId)
                {
                    const auto cmdId{ Dod::DataUtils::get(this->internalContext.sortedIndices, localElId) };
                    const auto cmd{ Dod::DataUtils::get(cmds, cmdId) };
                    ...
                    Dod::DataUtils::constructBack(this->renderCmdsContext.rtCommands, Types::Render::Cmd(transform1), true);
        ...
        for (int32_t tileId{}; tileId < needTextures; ++tileId)
        {
            ...
            Dod::DataUtils::populate(this->renderCmdsContext.commands, Types::Render::Cmd(transform2), true);

            char name[64]{};
            std::format_to_n(name, sizeof(name), "Tile{}Decals", tileId + 1);
            const auto rtkey{ std::hash<std::string_view>{}(name) };
            Dod::DataUtils::populate(this->renderCmdsContext.batchMaterial, rtkey, true);
            Dod::DataUtils::populate(this->renderCmdsContext.batchDepth, 0, true);
