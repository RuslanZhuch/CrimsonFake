...
namespace Game::ExecutionBlock
{
    ...
    {
        for (int32_t id{}; id < Dod::DataUtils::getNumFilledElements(names); ++id)
        {
            const auto currName{ Dod::DataUtils::get(names, id) };
    ...
    static void createTextures(Render* render) noexcept
    {
        ...
        for (int32_t elId{}; elId < Dod::DataUtils::getNumFilledElements(createTextureCmds); ++elId)
        {
            ProtoRenderer::texture_t newTexture;
            const auto cmd{ Dod::DataUtils::get(createTextureCmds, elId) };
            const auto bLoaded{ newTexture.loadFromFile("resources/textures/" + std::string(cmd.filename.internalData.data())) };

            Dod::DataUtils::emplaceBack(render->assetsContext.textures, std::move(newTexture), bLoaded);
            ProtoRenderer::texturePtr_t texturePtr{ &Dod::DataUtils::get(render->assetsContext.textures,
                Dod::DataUtils::getNumFilledElements(render->assetsContext.textures) - 1) };

            const auto name{ std::string_view(cmd.filename.internalData.data()) };
            const auto key{ std::hash<std::string_view>{}(name) };
            Dod::DataUtils::populate(render->assetsContext.loadedTextureIds, key, bLoaded);
            Dod::DataUtils::populate(render->assetsContext.loadedTextures, texturePtr, bLoaded);
        }
    }
    ...
    static void createRenderTextures(Render* render) noexcept
    {
        ...
        for (int32_t elId{}; elId < Dod::DataUtils::getNumFilledElements(createRenderTextureCmds); ++elId)
        {
            ...
            const auto cmd{ Dod::DataUtils::get(createRenderTextureCmds, elId) };

            Dod::DataUtils::constructBack(render->assetsContext.renderTextures);
            auto renderTexturePtr{ &Dod::DataUtils::get(render->assetsContext.renderTextures,
                Dod::DataUtils::getNumFilledElements(render->assetsContext.renderTextures) - 1) };
            ...
            const auto key{ std::hash<std::string_view>{}(name) };
            Dod::DataUtils::populate(render->assetsContext.renderTextureNames, key, bCreated);
            Dod::DataUtils::populate(render->assetsContext.loadedTextureIds, key, bCreated);
            Dod::DataUtils::populate(render->assetsContext.loadedTextures, &renderTexturePtr->getTexture(), bCreated);
        }
    }
    ...
    void Render::updateImpl([[maybe_unused]] float dt) noexcept
    {
        ...
        {
            Dod::DataUtils::populate(this->applicationContext.commands, 1, event.type == sf::Event::Closed);
        }
...

        {
            ...
            for (int32_t globalOffset{}, batchElId{}; batchElId < Dod::DataUtils::getNumFilledElements(batches); ++batchElId)
            {
                Dod::DataUtils::populate(this->internalContext.globalCmdOffsets, globalOffset, true);
                globalOffset += Dod::DataUtils::get(batches, batchElId).numOfCmds;
            }
            ...
            Dod::Algorithms::getSortedIndices(this->internalContext.priority, Dod::DataUtils::createImFromBuffer(depths));

            const auto renderTextures{ Dod::SharedContext::get(this->cmdsContext).rtBatchTarget };

            for (int32_t batchElId{}; batchElId < Dod::DataUtils::getNumFilledElements(batches); ++batchElId)
            {
                const auto batchId{ Dod::DataUtils::get(this->internalContext.priority, batchElId) };

                const auto batch{ Dod::DataUtils::get(batches, batchId) };
                const auto commandsInBatch{ batch.numOfCmds };

                const auto renderTextureId{ getTexture(
                    Dod::DataUtils::createImFromBuffer(this->assetsContext.renderTextureNames),
                    Dod::DataUtils::get(renderTextures, batchId)
                ) };

                if (renderTextureId < 0)
                    continue;

                auto& renderTexture{ Dod::DataUtils::get(this->assetsContext.renderTextures, renderTextureId) };

                const auto textureId{ getTexture(
                    Dod::DataUtils::createImFromBuffer(this->assetsContext.loadedTextureIds),
                    Dod::DataUtils::get(batchMaterials, batchId)
                ) };
                if (textureId < 0)
                    continue;

                auto texturePtr{ Dod::DataUtils::get(this->assetsContext.loadedTextures, textureId) };
                ...
                const auto globalCmdOffset{ Dod::DataUtils::get(this->internalContext.globalCmdOffsets, batchId) };
                for (int32_t cmdElId{}; cmdElId < commandsInBatch; ++cmdElId)
                {
                    const auto& transform{ Dod::DataUtils::get(commands, cmdElId + globalCmdOffset).transform };
